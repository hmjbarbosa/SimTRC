...
Nmass=30;
% Qual a massa inicial
mass0=me*0.5;
% Qual a variacao 
Dmass=me/Nmass;

disp(['% Y-tela(cm)   Massa/me ()    v0/c ()']);
for m=1:Nmass
  ...
  v0=sqrt(2*qe*Vac/mp);    % Velocidade inicial (m/s)
  ...
  %% ------------------------------------------------------------------------------%
  %% SALVA
  massY(m)=X(2); % posicao Y na tela
  massM(m)=mp;   % massa desta particula
  disp([num2str(massY(m)*100) '   ' num2str(massM(m)/me) '   ' ... 
       num2str(v0/c) ]);
